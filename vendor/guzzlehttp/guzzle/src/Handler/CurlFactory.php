...
namespace AltaPay\vendor\GuzzleHttp\Handler;

use AltaPay\vendor\GuzzleHttp\Exception\ConnectException;
use AltaPay\vendor\GuzzleHttp\Exception\RequestException;
use AltaPay\vendor\GuzzleHttp\Promise as P;
use AltaPay\vendor\GuzzleHttp\Promise\FulfilledPromise;
use AltaPay\vendor\GuzzleHttp\Promise\PromiseInterface;
use AltaPay\vendor\GuzzleHttp\Psr7\LazyOpenStream;
use AltaPay\vendor\GuzzleHttp\TransferStats;
use AltaPay\vendor\GuzzleHttp\Utils;
use AltaPay\vendor\Psr\Http\Message\RequestInterface;
use AltaPay\vendor\Psr\Http\Message\UriInterface;
/**
...
class CurlFactory implements CurlFactoryInterface
{
    ...
    private static function createRejection(EasyHandle $easy, array $ctx) : PromiseInterface
    {
        ...
        if ('' !== $sanitizedError) {
            $redactedUriString = \AltaPay\vendor\GuzzleHttp\Psr7\Utils::redactUserInfo($uri)->__toString();
    ...
    private static function sanitizeCurlError(string $error, UriInterface $uri) : string
    {
        ...
        $redactedUriString = \AltaPay\vendor\GuzzleHttp\Psr7\Utils::redactUserInfo($baseUri)->__toString();
    ...
    private function applyHandlerOptions(EasyHandle $easy, array &$conf) : void
    {
        ...
        if (!isset($options['sink'])) {
            ...
            $options['sink'] = \AltaPay\vendor\GuzzleHttp\Psr7\Utils::tryFopen('php://temp', 'w+');
        }
        $sink = $options['sink'];
        if (!\is_string($sink)) {
            $sink = \AltaPay\vendor\GuzzleHttp\Psr7\Utils::streamFor($sink);
